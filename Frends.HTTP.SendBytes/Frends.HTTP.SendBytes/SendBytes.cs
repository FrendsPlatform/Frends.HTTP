...
[assembly: InternalsVisibleTo("Frends.HTTP.SendBytes.Tests")]
...
public class HTTP
{
    ...
    public static async Task<Result> SendBytes([PropertyTab] Input input, [PropertyTab] Options options, CancellationToken cancellationToken)
    {
        ...
        using var content = new ByteArrayContent(input.ContentBytes);
        ...
        var response = new Result(await responseMessage.Content.ReadAsStringAsync().ConfigureAwait(false), GetResponseHeaderDictionary(responseMessage.Headers, responseMessage.Content.Headers), (int)responseMessage.StatusCode);
